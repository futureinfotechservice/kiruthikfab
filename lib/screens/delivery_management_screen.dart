...
class _DeliveryManagementListScreenState
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      final data = await DeliveryManagementApiService().getAllProducts(
        companyId: companyid,
      );
      ...
      if (data['status'] == 'success' && data['delivery_items'] != null) {
        ...
        setState(() {
          allRecords = records;
          allGroups = DeliveryRecord.groupRecords(records);
        ...
        _showSnackBar("Failed to load records", isError: true);
      }
    } catch (e) {
      setState(() {
        loading = false;
  ...
  void applyFilters() {
    List<DeliveryRecordGroup> temp = List.from(allGroups);

    // Bill No Filter
    if (billController.text.trim().isNotEmpty) {
      temp = temp.where((group) {
        return group.invoiceNo.toLowerCase().contains(
          billController.text.toLowerCase(),
        );
      }).toList();
    }
    ...
    if (fromDate != null) {
      temp = temp.where((group) {
        DateTime d = DateTime.parse(group.date);
