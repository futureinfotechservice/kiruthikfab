...
class _SourceEntryScreenState extends State<SourceEntryScreen> {
  ...
  final TextEditingController _backgroundNetworkController = TextEditingController();
  final TextEditingController _customerInterestController = TextEditingController();
  ...
  void _loadSourceData() {
    if (widget.source != null && _isDropdownDataLoaded) {
      setState(() {
        ...
        _selectedAreaId = widget.source!.areaId.isNotEmpty ? widget.source!.areaId : null;
        _selectedOccupationId = widget.source!.occupationId.isNotEmpty ? widget.source!.occupationId : null;
        _selectedReferById = widget.source!.referById.isNotEmpty ? widget.source!.referById : null;
        _selectedAgentId = widget.source!.agentId.isNotEmpty ? widget.source!.agentId : null;
        _selectedSourcingModeId = widget.source!.sourcingModeId;
        _selectedEntryPersonId = widget.source!.entryPersonId.isNotEmpty ? widget.source!.entryPersonId : null;
        _selectedSalesPersonId = widget.source!.salesPersonId.isNotEmpty ? widget.source!.salesPersonId : null;
      });
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    if (_selectedDistrict == null || _selectedDistrict!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select Branch'), backgroundColor: Colors.red),
      );
      return;
    }
    if (_selectedSourcingModeId == null || _selectedSourcingModeId!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select Sourcing Mode'), backgroundColor: Colors.red),
      );
      return;
    }
    ...
    try {
      ...
      if (result == "Success") {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_isEditMode ? 'Source updated successfully!' : 'Source created successfully!'),
