...
import '../services/size_apiservice.dart';

class SizeMasterScreen extends StatefulWidget {
  const SizeMasterScreen({super.key});
...
class _SizeMasterScreenState extends State<SizeMasterScreen> {
  ...
  final SizeApiService _apiService = SizeApiService();
  ...
  // Store the size being edited
  SizeMasterModel? _editingSize;

  List<SizeMasterModel> _sizes = [];
  String _searchQuery = '';
  ...
  Future<void> _loadSizes() async {
    ...
    try {
      final sizes = await _apiService.fetchSizes(context);
      if (mounted) {
        setState(() {
          _sizes = sizes;
        });
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (_isEditMode && _editingSize != null) {
        // Update existing size
        result = await _apiService.updateSize(
          context: context,
          sizeId: _editingSize!.id,
          sizename: _sizeNameController.text,
        );
      } else {
        // Insert new size
        result = await _apiService.insertSize(
          context: context,
          sizename: _sizeNameController.text,
        );
      }
  ...
  Future<void> _deleteSize(String sizeId, int index) async {
    ...
    if (confirmed == true && mounted) {
      final result = await _apiService.deleteSize(context, sizeId);
      if (result == "Success" && mounted) {
        setState(() {
          _sizes.removeAt(index);
          // If we were editing this size, cancel edit mode
          if (_editingSize?.id == sizeId) {
            _cancelEdit();
          }
        });
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _editSize(SizeMasterModel size) {
    setState(() {
      _isEditMode = true;
      _editingSize = size;
  ...
  List<SizeMasterModel> get _filteredSizes {
    if (_searchQuery.isEmpty) return _sizes;
    return _sizes.where((size) {
      return size.sizename.toLowerCase().contains(_searchQuery.toLowerCase());
    }).toList();
  }
  ...
  Widget _buildHeaderCard() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            _isEditMode ? 'Edit Size' : 'Add New Size',
          ...
          Text(
            _isEditMode
                ? 'Updating: ${_editingSize?.sizename ?? ''}'
                      ...
                      Container(
                        ...
                        child: TextFormField(
                          ...
                          decoration: InputDecoration(
                            hintText: 'Enter size name (e.g., Small, Medium, Large, XL, 32, 34, etc.)',
                          ...
                          validator: (value) {
                            ...
                            if (value.length < 1) {
                              return 'Size name must be at least 1 character';
                            }
                            return null;
                          },
                        ),
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text(
                        _isEditMode ? 'Update Size' : 'Create Size',
                        style: const TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.w500,
                          color: Colors.white,
        ...
        children: [
          ...
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final bool isEditing = _editingSize?.id == size.id;

              return Container(
                decoration: BoxDecoration(
                  color: isEditing ? Colors.blue.shade50 : null,
                  border: index < _filteredSizes.length - 1
                      ? const Border(
                    bottom: BorderSide(color: Color(0xFFE2E8F0)),
                  )
                      : null,
                    ...
                    Expanded(
                      ...
                      child: Row(
                        ...
                        children: [
                          // Edit button
                          IconButton(
                            onPressed: isEditing ? null : () => _editSize(size),
                            icon: Icon(
                              Icons.edit,
                          ...
                          // Delete button
                          IconButton(
                            onPressed: () => _deleteSize(size.id, index),
                            icon: const Icon(
                              Icons.delete,
                              color: Colors.red,
                              size: 20,
