...
try {
    ...
    $stmt = $conn->prepare("
        INSERT INTO invoice_head (invoiceno, companyid, customerid, date, remarks, taxpercentage, subtotal, grandtotal, addedby, status,packing_amount)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)
    ");
    $stmt->bind_param("siisssssssi", $invoiceno, $companyid, $customerid, $date, $remarks, $taxpercentage, $subtotal, $grandtotal, $addedby, $status,$packing_amount);
    $stmt->execute();
