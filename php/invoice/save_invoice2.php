...
$gst_no  = $obj["gst_no"];
$delivery_partner  = $obj["delivery_partner"];
$conn->begin_transaction();

try {
    // Insert into invoice_head
    $stmt = $conn->prepare("
        INSERT INTO invoice_head (invoiceno, companyid, customerid, date, remarks, taxpercentage, subtotal, grandtotal, addedby, status,packing_amount,gst_no,delivery_partner)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)
    ");
    $stmt->bind_param("siisssssssiss", $invoiceno, $companyid, $customerid, $date, $remarks, $taxpercentage, $subtotal, $grandtotal, $addedby, $status,$packing_amount,$gst_no,$delivery_partner);
    $stmt->execute();
