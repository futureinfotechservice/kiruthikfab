...
if (!empty($obj)) {
    ...
    $refer_id = mysqli_real_escape_string($conn, $obj['refer_id'] ?? '');
    $incharge = mysqli_real_escape_string($conn, $obj['incharge'] ?? '');
    $incharge_id = mysqli_real_escape_string($conn, $obj['incharge_id'] ?? '');
    ...
    $salesperson_id = mysqli_real_escape_string($conn, $obj['salesperson_id'] ?? '');
    ...
    $incharge_id = mysqli_real_escape_string($conn, $_POST['incharge_id'] ?? '');
    ...
    $salesperson_id = mysqli_real_escape_string($conn, $_POST['salesperson_id'] ?? '');
...
$required_fields = ['customername', 'address', 'area_id', 'mobile1'];
...
// Insert query with ID fields
$sql = "INSERT INTO customermaster (
    companyid, customername, gst_no, address, area, area_id, areaid, 
    mobile1, mobile2, whatsapp, refer, refer_id, incharge, incharge_id, 
    agent, agent_id, salesperson, salesperson_id, occupation, occupation_id,
    aadharurl, photourl, addedby, activestatus
) VALUES (
    '$companyid', '$customername', '$gst_no', '$address', '$area', '$area_id', '$areaid',
    '$mobile1', '$mobile2', '$whatsapp', '$refer', '$refer_id', '$incharge', '$incharge_id',
    '$agent', '$agent_id', '$salesperson', '$salesperson_id', '$occupation', '$occupation_id',
