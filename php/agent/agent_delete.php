...
if (empty($agentid) || empty($companyid)) {
    echo json_encode(["status" => "error", "message" => "Agent ID and Company ID are required"]);
    mysqli_close($conn);
    exit();
}

// Soft delete - update activestatus to 0
// $sql = "UPDATE agentmaster SET activestatus = '0' WHERE id = '$agentid' AND companyid = '$companyid'";
$sql = "Delete from agentmaster WHERE id = '$agentid' AND companyid = '$companyid'";
